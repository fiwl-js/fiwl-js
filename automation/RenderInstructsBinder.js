...
function RenderInstructsBinder(isVerbose = false) {
  ...
  if (!isVerbose)
    console.log(' - Reading "./src/display/renderer/instructs/*.ts"');
  ...
  for (let iter = 0; iter < instructStringList.length; iter++) {
    ...
    if (isVerbose) {
      console.log(
        ` - Loaded: ${eachInstructPointer.className} as ${eachInstructPointer.key}`
      );
    }
...
function deleteOldFile() {
  const filePath = path.resolve(
    __dirname,
    "../src/display/renderer/RenderAPI.ts"
  );
...
function getAllInstructs(isVerbose) {
  const directoryPath = path.resolve(
    __dirname,
    "../src/display/renderer/instructs"
  );
      ...
      for (let iter = startPos; iter < instructString.length; iter++) {
        ...
        if (result.length == 0 && !isNaN(eachChar)) {
          throw new SyntaxError(
            "Key of " + className + " is started with number"
          );
        }
...
function buildParamPointers(instructString, className) {
  ...
  for (let iter = 0; iter < constructorArgsArray.length; iter++) {
    const eachArg = constructorArgsArray[iter]
      .trim()
      .replace(/\%\&COMMA\$/g, ",");
    ...
    const type =
      identifierTokenized.length > 1 ? identifierTokenized[1].trim() : "any";
