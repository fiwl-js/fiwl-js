...
export default class MouseEventDispatcher implements DispatcherTemplate {
  ...
  attach(
    ...
    this.middleDoubleListeners = listenerMap.get(
      EventTypes.MOUSE_MIDDLE_DOUBLE
    );
    this.middleReleaseListeners = listenerMap.get(
      EventTypes.MOUSE_MIDDLE_RELEASE
    );
    ...
    this.rightReleaseListeners = listenerMap.get(
      EventTypes.MOUSE_RIGHT_RELEASE
    );
  ...
  private triggerMove = (domEventData: MouseEvent): void => {
    ...
    if (
      ...
      this.hoverStartListeners.length == 0 &&
      this.hoverEndListeners.length == 0
    )
      return;
