...
export default class TouchEventDispatcher implements DispatcherTemplate {
    ...
    private triggerStart = (domEventData:TouchEvent):void => {
        ...
        if(this.startListeners.length == 0 && domEventData.touches.length != 1) return;
        ...
        if(isNaN(this.holdHandler)) {
            if(fingers.length == 1) {
                this.holdHandler = window.setTimeout(() => this.triggerLong(eventData), 1000);
            }
        } else {
        ...
        for(let iter = 0; iter < this.startListeners.length; iter++) {
            ...
            if(eachListener.target) {
                ...
                if(EventManager.isInTarget(target, currentFinger.clientX, currentFinger.clientY)) {
