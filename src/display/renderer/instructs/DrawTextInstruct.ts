...
export default class DrawTextInstruct implements InstructTemplate {
    ...
    constructor(text:string, x:number, y:number, size:number, colorHex:number, styles:string[] = [], font:Font = null, horizontalAlign:'left'|'center'|'right' = 'left') {
        this.text = text;
        this.x = x;
        this.y = y;
        this.size = size;
    ...
    renderWithCustomFont(context:CanvasRenderingContext2D) {
        ...
        if(this.text != text && !DrawTextInstruct.hadBeenWarned)  {
            console.warn("WARNING: Custom font does not support multi-line render!\nplease split lines on your own.");
            DrawTextInstruct.hadBeenWarned = true;
        }

        this.font.draw(context, text, this.x, this.y + (this.size*2/3), this.size);
    }
}