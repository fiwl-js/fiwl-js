...
export default class StrokePathInstruct implements InstructTemplate {
  ...
  render(context: CanvasRenderingContext2D) {
    ...
    context.beginPath();
    context.lineWidth = isNaN(first.thickness)
      ? this.thickness
      : first.thickness;
    ...
    for (let iter = 1; iter < this.vertices.length; iter++) {
      ...
      if (currentVertex.smooth === true) {
        ...
        if (iter < this.vertices.length - 1) {
          ...
          context.lineWidth = isNaN(nextVertex.thickness)
            ? this.thickness
            : nextVertex.thickness;
          ...
          context.lineWidth = isNaN(currentVertex.thickness)
            ? this.thickness
            : currentVertex.thickness;
          context.arcTo(x1, y1, x2, y2, distance / 2);
        }
      } else {
        context.lineWidth = isNaN(currentVertex.thickness)
          ? this.thickness
          : currentVertex.thickness;
        const isBezier =
          !isNaN(prevVertex.nextBezierX) ||
          !isNaN(prevVertex.nextBezierY) ||
          !isNaN(currentVertex.prevBezierX) ||
          !isNaN(currentVertex.prevBezierY);
        if (isBezier) {
          let cp1x = isNaN(prevVertex.nextBezierX)
            ? prevVertex.x
            : prevVertex.nextBezierX;
          let cp1y = isNaN(prevVertex.nextBezierY)
            ? prevVertex.y
            : prevVertex.nextBezierY;
          let cp2x = isNaN(currentVertex.prevBezierX)
            ? currentVertex.x
            : currentVertex.prevBezierX;
          let cp2y = isNaN(currentVertex.prevBezierY)
            ? currentVertex.y
            : currentVertex.prevBezierY;
