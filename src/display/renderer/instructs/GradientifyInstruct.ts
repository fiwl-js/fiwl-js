...
export default class GradientifyInstruct implements InstructTemplate {
    ...
    constructor(target:InstructTemplate, startX:number, startY:number, endX:number, endY:number, gradientPoints:Array<GradientPointTemplate>, isRadial:boolean = false) {
        this.target = target;
        this.startX = startX;
        this.startY = startY;
        this.endX = endX;
    ...
    render(context:CanvasRenderingContext2D) {
        ...
        if(this.isRadial) {
            const radius = Math.sqrt(((this.endX-this.startX)**2) + ((this.endY-this.startY)**2));
            gradient = context.createRadialGradient(
                this.startX, this.startY, 0, this.startX, this.startY, radius
            );
        } else {
