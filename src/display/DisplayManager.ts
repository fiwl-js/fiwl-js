...
function startBootAnim(splashURL:string):Promise<void> {
    return new Promise(resolve => {
        if(splashURL.toLowerCase().endsWith('.svg')) {
            NetworkManager.getImage(splashURL).then(result => {
...
function render():void {
    ...
    for(let iter = 0; iter < layers.length; iter++) {
        const eachLayer = layers[iter];
        let instructs = api.group(eachLayer.drawFunc.call(eachLayer.objectContext,api)) as InstructTemplate;
        ...
        if(eachLayer.x != 0 || eachLayer.y != 0) {
            instructs = api.translate(
                instructs,
                eachLayer.x, eachLayer.y
            );
        }
...
function setCursor(name:'default'|'processing'|'clickable'|'text'|'move'|'draggable'|'dragging'|'denied'):void {
