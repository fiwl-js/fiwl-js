...
async function bindExposables(
  target: FIWLApplication,
  exposableNames: Array<string>
): Promise<void> {
  if (!paramsMap.has(target)) {
    throw new ReferenceError(
      "[INTERNAL] App instance is not registered in paramsMap"
    );
  }
  ...
  for (let iter = 0; iter < exposableNames.length; iter++) {
    ...
    if (typeof eachPath == "string") {
      ...
      throw new ReferenceError(
        `[INTERNAL] Exposable with "${eachName}" name is not exist`
      );
    }
  }
}
