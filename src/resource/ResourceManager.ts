...
function loadStyle(resURL:string):Promise<FIWLStyle> {
    ...
    promiseList.push(NetworkManager.getVARL('/style.varl').then(globalResponse => {
    ...
    if(resURL != '' && resURL != '/') {
        promiseList.push(NetworkManager.getVARL(resURL+'/style.varl').then(stageResponse => {
...
async function loadIcon(resURL:string, name:string):Promise<ImageBitmapSource> {
    resURL = resURL.trim().replace(/\/$/, '');
    if(resURL != '' && resURL != '/') {
        try {
...
async function loadPicture(resURL:string, name:string, extension:string = 'jpg'):Promise<ImageBitmapSource> {
    resURL = resURL.trim().replace(/\/$/, '');
    if(name.match(/\..*$/)) warnExtension('loadPicture', name);
    if(resURL != '' && resURL != '/') {
        try {
            return await NetworkManager.getImage(resURL+'/pictures/'+name+'.'+extension);
        } catch(error) {
...
async function loadFont(resURL:string, name:string, extension:string = 'ttf'):Promise<Font> {
    resURL = resURL.trim().replace(/\/$/, '');
    if(name.match(/\..*$/)) warnExtension('loadFont', name);
    if(resURL != '' && resURL != '/') {
        try {
            return await NetworkManager.getFont(resURL+'/fonts/'+name+'.'+extension);
        } catch(error) {
...
function warnExtension(funcName:string, nameAndExt:string):void {
    ...
    console.warn(`WARNING: Change app.res.${funcName}("${name}.${extension}") to app.res.${funcName}("${name}", "${extension}")`);
}
