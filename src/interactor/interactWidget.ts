...
// TODO: Find way to assign multiple definition type without ts-ignore
...
export default function interactWidget(
  ...
  if (passiveWidget instanceof Layout) {
    ...
    for (let iter1 = 0; iter1 < internal.contents.length; iter1++) {
      ...
      // @ts-ignore
      interactiveItem.constructor.internal.get(
        interactiveItem
      ).container = result;
    }
  ...
  for (let iter = 0; iter < keys.length; iter++) {
    ...
    if (
      // @ts-ignore
      typeof passiveWidget[eachKey] == "function" ||
      // @ts-ignore
      passiveWidget.constructor.flagNoUpdate.includes(eachKey)
    ) {
  ...
  const flagForwardWeakMap: Array<WeakMap<DisplayObject, any>> =
    // @ts-ignore
    passiveWidget.constructor.flagForwardWeakMap;
